<?php
  ...
  function calendario_pagos( $id_alu_ram, $db ){
    // FUNCIÓN QUE CARGA PAGOS DE COLEGIATURA E INSCRIPCIÓN A MATRÍCULAS

    // EXTRAER id_alu_ram A PARTIR DEL pago ASÍ COMO COSTO DE COLEGIATURA
    $sqlMatricula = "
        SELECT * 
        FROM vista_pagos
    ...
    $conteo_colegiaturas = $datos['conteo_colegiaturas'];
    $conteo_tramites = $datos['conteo_tramites'];
    $conteo_inscripciones = $datos['conteo_inscripciones'];

    $ini_pag_aux = $datos['ini_pag'];

    //**ADICIONAR COSTO DE CERTIFICACIÓN Y VALIDAR SI SE DIVIDE ENTRE 2 PAGOS IDÉNTICOS PARA TRÁMITE 1 Y TRÁMITE 2 SI APLICA PARA TODOS LAS MODALIDADES */
    //$cor_ram = $datos['cos_ram'];
    $tra1_ram = 3500;
    $tra2_ram = 3500;

    $cos_ram = 1000;

    // VALIDAR CUÁNTAS COLEGIATURAS YA EXISTEN ASOCIADAS A LA MATRÍCULA
    
    // INICIA ALGORITMO
    if( $estatus_academico != 'Egresado' && ($estatus_general != 'Bloqueado' || $estatus_general == 'Suspendido' ) ){
    //
        // CASO COLEGIATURA 1
        //**VALIDAR DE PREFERENCIA SI EXISTE O NO INSCRIPCIÓN O EN UN UPDATE*/
        if( $conteo_colegiaturas == 0 ){
            ...
            $mon_ori_pag = $cos_ram;
            $mon_pag = $mon_ori_pag;
            $con_pag = 'Colegiatura '.($conteo_colegiaturas+1); // CONTEO DE COLEGIATURAS
            $est_pag = 'Pendiente';
            $res_pag = 'Sistema';
            $ini_pag = restarDias( $ini_pag, 7 );
            $fin_pag = $ini_pag;
            $tip_pag = 'Colegiatura';
            //    
        // CASO COLEGIATURA 2
        // } else if ( $conteo_colegiaturas == 1 ){
            
            // VALIDAR EL INICIO EN FUNCIÓN DEL DÍA
            // $fechaInicio = $ini_gen;
            // $fechaInicioAux = new DateTime($fechaInicio);
            // $dia = $fechaInicioAux->format('d');

            // // RETORNA EL DÍA EN NÚMERO EJEM: 23nov23 = 23
            // $dia = (int)$fechaInicioAux->format('d');
            // SI ESTÁ ENTRE 1 Y 15
            // if( $dia >= 1 && $dia <= 15 ){
                ...
                // $est_pag = 'Pendiente';
                // $res_pag = 'Sistema';
                // $ini_pag = cambiarDiaFecha(sumarUnMes($ini_pag_aux), 1);
                // $fin_pag = sumarDias( $ini_pag, 4 );
                // $tip_pag = 'Colegiatura';

            // SI ESTÁ DEL 16 AL FIN DE MES
            // } else {
                ...
                // $est_pag = 'Pendiente';
                // $res_pag = 'Sistema';
                // $ini_pag = cambiarDiaFecha(sumarUnMes($ini_pag_aux), 10);
                // $fin_pag = cambiarDiaFecha(sumarUnMes($ini_pag_aux), 15);
                // $tip_pag = 'Colegiatura';

            // }

        // CASO DESPUÉS DE LA 2 COLEGIATURA CALENDARIO REGULAR DEL 1 AL 5 PARA TODAS
        } else {
            // COLEGIATURAS 3 EN ADELANTE

            // TRÁMITES 1 Y 2
            // echo "coles: ".$conteo_colegiaturas.'------- trams: '.$conteo_tramites;
            if( ( $conteo_tramites != 2 ) && ( $conteo_colegiaturas == 1 || $conteo_colegiaturas == 2 ) ) {
                // **VALIDAR SI SE COLOCA MANUALMENTE UNA COLEGIATURA FUERA DEL ALGORITMO SE BRINCARA LA GENERACIÓN DEL TRÁMITE
                // VARIABLES

                //echo 'condicion tramites';

                if( $conteo_tramites == 0 ){
                    $tramite = $tra1_ram;
                } else if ( $conteo_tramites == 1 ){
                    $tramite = $tra2_ram;
                }
                $id_alu_ram10 = $id_alu_ram;
                $fec_pag = date('Y-m-d');
                $mon_ori_pag = $tramite;
                $mon_pag = $mon_ori_pag;
                $con_pag = 'Trámite '.($conteo_tramites+1); // CONTEO
                $est_pag = 'Pendiente';
                $res_pag = 'Sistema';
                $ini_pag = cambiarDiaFecha(sumarUnMes($ini_pag_aux), 14);
                $fin_pag = encontrarProximoDia( $ini_gen, $ini_pag );
                $tip_pag = 'Otros';
    ...
    // MIENTRAS QUE EL ALUMNO NO SUPERE LA FECHA DE FIN DE SU GENERACIÓN Y NO TENGA UNA CALIFICACIÓN APROBATORIA
    // FIN ALGORITMO
...
<?php
  ...
  function enviarCorreoPagoAlumnoServer( $id_pag, $nombrePlantel, $correo2Plantel, $ligaPlantel, $esloganPlantel, $nomResponsable, $direccionPlantel ){
    ...
    $mensaje = '
      ...
      $cabeceras .= 'From: '.$nombrePlantel.'<admisiones@ahjende.com>'."\r\n";
...
<?php
  ...
  function obtenerDatosAlumnoPrograma( $id_alu_ram ){
    ...
    function duplicarContenidoProgramaMateriaAMateria( $db, $materia_emisora, $materia_receptora ) {   
      // BLOQUESS
