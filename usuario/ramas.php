...
<div class="row">
	...
	<?php  
		$sqlRamas = "SELECT * FROM rama INNER JOIN plantel ON plantel.id_pla = rama.id_pla1 WHERE id_pla1 = '$plantel' ORDER BY id_ram DESC";
		// $sqlRamas = "SELECT * FROM rama INNER JOIN plantel ON plantel.id_pla = rama.id_pla1 WHERE id_cad1 = '1' ORDER BY id_ram DESC";
	...
	<table id="myTable" class="table table-hover table-striped table-sm" cellspacing="0" width="100%">
		<thead class="bg-info text-white">
			<tr>
				...
				<th class="letraPequena font-weight-normal">Visibilidad en formulario</th>
		...
		?>
			<tr>
				...
				<td class="letraPequena font-weight-normal"><?php echo $filaRamas['nom_ram'].'---'.$filaRamas['nom_pla']; ?></td>
				...
				<td class="letraPequena font-weight-normal">
					<?php  
						if ( $filaRamas['est_ram'] == 'Activo' ) {
					?>
							<div class="row">
								<div class="col-md-12 text-center">
									<a class="btn-floating btn-sm lime accent-4 switchRamas" id_ram="<?php echo $id_ram; ?>" id_ram="<?php echo $id_ram; ?>" estatus="<?php echo $filaRamas['est_ram']; ?>" title="Activa/Desactiva la visibilidad del programa en el formulario del alumno...">
										<i class="fas fa-power-off"></i>
									</a>
								</div>
							</div>
					<?php
						} else if ( $filaRamas['est_ram'] == 'Inactivo' ) {
					?>

							<div class="row">
								<div class="col-md-12 text-center">
									<a class="btn-floating btn-sm btn-secondary switchRamas" id_ram="<?php echo $id_ram; ?>" id_ram="<?php echo $id_ram; ?>" estatus="<?php echo $filaRamas['est_ram']; ?>" title="Activa/Desactiva la visibilidad del programa en el formulario del alumno...">
										<i class="fas fa-power-off"></i>
									</a>
								</div>
							</div>
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body mx-3">
	        ...
	        <div style="position: relative;">
				...
				<span class="letraPequena grey-text" style="position: absolute; top: -15px;">Modalidad</span>
		        <select class="browser-default custom-select" id="mod_ram" name="mod_ram" required>
			...
			<div class="" style="position: relative;">
				...
				<span class="letraPequena grey-text" style="position: absolute; top: -15px;">Nivel académico</span>
				<select class="browser-default custom-select" id="gra_ram" name="gra_ram" required>
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body mx-3">
			...
			<div class="" style="position: relative;">
				...
				<span class="letraPequena grey-text" style="position: absolute; top: -15px;">Nivel académico</span>
				<select class="browser-default custom-select" id="nivelEducativo" name="nivelEducativo" required>
				  	<option value="Licenciatura">Licenciatura</option>
				  	<option value="Preparatoria">Preparatoria</option>
				  	<option value="Maestría">Maestría</option>
				  	<option value="Diplomado">Diplomado</option>
	...
	$('.edicion').on('click', function(){
		...
		$.ajax({
			...
			success: function(datos){
				...
				$('#modalidad').val( datos.mod_ram );
				$('#nivelEducativo').val( datos.gra_ram );
...
<script>
	$(".switchRamas").on('click', function(event) {
		event.preventDefault();
		/* Act on the event */
		var elemento = $(this);
		...
		// console.log( estatus + id_ram );
		$.ajax({
			url: 'server/editar_estatus_rama.php',
			type: 'POST',
			data: { id_ram, estatus },
