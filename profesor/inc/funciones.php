<?php
  ...
  // function actualizar_registro_cal_act_server( $id_alu_ram, $id_ent_cop ){
  ...
  //VALIDACION SI YA EXISTE REGISTRO EN CALIFICACIONES
  function validacion_insert( $id_alu_ram, $id_mat ){
    require('../../includes/conexion.php');
    ...
    $resultado = mysqli_query( $db, $get_calificaciones );
  ...
  // ELIMINA REGISTROS DE CALIFICACIONES DUPLICADOS DE LOS ALUMNOS
  function remover_duplicado_alumno( $id_alu_ram, $id_mat ){
    require('../../includes/conexion.php');

    $get_calificaciones = "
                      SELECT count(id_cal) as registros
                      FROM calificacion
                      WHERE id_alu_ram2 = '$id_alu_ram' AND id_mat4 = '$id_mat'
                    ";

    $resultado = mysqli_query( $db, $get_calificaciones );
    $get_count = mysqli_fetch_assoc($resultado);
    $resultado = $get_count['registros'];
    //echo "alumno involucrado: ".$get_calificaciones.'<br>'.'reg duplicados: '.$resultado;
    if($resultado>1){
      //echo 'eliminacion para alu_ram'.$id_alu_ram.'<br>';
      $cantidad = $resultado-1;
      $delete_duplicado = "DELETE FROM calificacion WHERE id_alu_ram2 = $id_alu_ram AND id_mat4 = $id_mat  order by fin_cal ASC limit $cantidad";
      $run = mysqli_query($db, $delete_duplicado);
      //echo $delete_duplicado.';<br>';
  ...
  // OBTENER DATOS VISTA ALUMNO
  function obtener_datos_vista_alumno( $id_alu_ram ){
    require('../includes/conexion.php');
  ...
  function obtener_conteo_notificaciones_usuario_server( $id, $tipo ){
    ...
    $sql = "
      ...
      WHERE ( use_est_men = '$id' AND tip_est_men = '$tipo' ) AND est_est_men = 'Entregado'
    ";
  ...
  function obtenerTipoUsuarioCompletoAlumnoServer( $id_alu ){
    require('../../includes/conexion.php');
    ...
    $cadena = '<span class="badge-pill badge-info font-weight-normal letraPequena">Alumno</span>';
    while( $fila = mysqli_fetch_assoc( $resultado ) ){
      $cadena = $cadena.' <span class="badge-pill badge-primary font-weight-normal letraPequena">'.$fila['nom_gen'].'</span>';
    }
  ...
  function obtenerColorDesempennioAlumnoServer( $id_alu_ram ){
    require('../../includes/conexion.php');

    $fechaHoy = date( 'Y-m-d' );

    $sql = "
      SELECT id_for_cop AS identificador_copia, nom_for AS actividad, pun_for AS puntaje, ini_for_cop AS inicio, fin_for_cop AS fin, tip_for AS tipo_actividad, id_alu_ram AS id_alu_ram, id_for_cop AS id_cop, fec_cal_act AS fecha, nom_mat AS nom_mat, id_blo AS id_blo, id_sub_hor AS id_sub_hor, nom_gru AS nom_gru
      FROM cal_act
      ...
      WHERE  fin_for_cop <= '$fechaHoy' AND id_alu_ram4 = '$id_alu_ram' AND fec_cal_act IS NULL
          UNION
      SELECT id_ent_cop AS identificador_copia, nom_ent AS actividad, pun_ent AS puntaje, ini_ent_cop AS inicio, fin_ent_cop AS fin, tip_ent AS tipo_actividad, id_alu_ram AS id_alu_ram, id_ent_cop AS id_cop, fec_cal_act AS fecha, nom_mat AS nom_mat, id_blo AS id_blo, id_sub_hor AS id_sub_hor, nom_gru AS nom_gru
      FROM cal_act
      ...
      INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
      WHERE  fin_ent_cop <= '$fechaHoy' AND id_alu_ram4 = '$id_alu_ram' AND fec_cal_act IS NULL
      UNION
          SELECT id_exa_cop AS identificador_copia, nom_exa AS actividad, pun_exa AS puntaje, ini_exa_cop AS inicio, fin_exa_cop AS fin, tip_exa AS tipo_actividad, id_alu_ram AS id_alu_ram, id_exa_cop AS id_cop, fec_cal_act AS fecha, nom_mat AS nom_mat, id_blo AS id_blo, id_sub_hor AS id_sub_hor, nom_gru AS nom_gru
      FROM cal_act
      ...
      INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
      WHERE  fin_exa_cop <= '$fechaHoy' AND id_alu_ram4 = '$id_alu_ram' AND fec_cal_act IS NULL
      ORDER BY inicio
    "; 
  ...
  function obtenerPuntajeExamenServer( $id_exa ){
    require('../../includes/conexion.php');
  ...
  function obtenerDatosActividadAlumnoServer( $identificador_copia, $tipo, $id_alu_ram ){
    require('../../includes/conexion.php');
    ...
    if ( $tipo == 'Foro' ) {
      ...
      $sql = "
        SELECT nom_for AS nom_act, nom_mat AS nom_mat, nom_gru AS nom_gru, nom_ram AS nom_ram
        FROM foro_copia
        INNER JOIN foro ON foro.id_for = foro_copia.id_for1
        INNER JOIN bloque ON bloque.id_blo = foro.id_blo4
        INNER JOIN materia ON materia.id_mat = bloque.id_mat6
        ...
        INNER JOIN sub_hor ON sub_hor.id_sub_hor = foro_copia.id_sub_hor2
        INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
        WHERE id_for_cop = '$identificador_copia'
      ";

    } else if ( $tipo == 'Entregable' ) {

      $sql = "
        SELECT nom_ent AS nom_act, nom_mat AS nom_mat, nom_gru AS nom_gru, nom_ram AS nom_ram
        ...
        INNER JOIN sub_hor ON sub_hor.id_sub_hor = entregable_copia.id_sub_hor3
        INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
        WHERE id_ent_cop = '$identificador_copia'
      ";

    } else if ( $tipo == 'Examen' ) {

      $sql = "
        SELECT nom_exa AS nom_act, nom_mat AS nom_mat, nom_gru AS nom_gru, nom_ram AS nom_ram
        FROM examen_copia
        INNER JOIN examen ON examen.id_exa = examen_copia.id_exa1
        INNER JOIN bloque ON bloque.id_blo = examen.id_blo6
        INNER JOIN materia ON materia.id_mat = bloque.id_mat6
        ...
        INNER JOIN sub_hor ON sub_hor.id_sub_hor = examen_copia.id_sub_hor4
        INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
        WHERE id_exa_cop = '$identificador_copia'
      ";
    ...
    $datos['nom_act'] = $fila['nom_act'];
    $datos['nom_mat'] = $fila['nom_mat'];
    $datos['nom_gru'] = $fila['nom_gru'];
    $datos['nom_ram'] = $fila['nom_ram'];
    ...
    $datos['nom_alu'] = $filaAlumno['app_alu']." ".$filaAlumno['apm_alu']." ".$filaAlumno['nom_alu'];
  ...
  function obtenerDescripcionCalificacionActividadLogServer( $tipo_usuario, $nombre_usuario, $accion, $tipo_actividad, $nom_act, $nom_mat, $nom_gru, $nom_ram, $nom_alu ){
    $fechaHoy = date( 'Y-m-d H:i:s' );


    if ( $tipo_usuario == 'Admin' ) {
          
          $des_log = 'El Administrador: '.$nombre_usuario." ".$accion." un ".$tipo_actividad." ( ".$nom_act." ), de la materia de ".$nom_mat.", del grupo ".$nom_gru.", en el programa: ".$nom_ram.". Del alumno: ".$nom_alu.". El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
            

        } else if ( $tipo_usuario == 'Adminge' ) {

            $des_log = 'El Gestor Escolar: '.$nombre_usuario." ".$accion." un ".$tipo_actividad." ( ".$nom_act." ), de la materia de ".$nom_mat.", del grupo ".$nom_gru.", en el programa: ".$nom_ram.". Del alumno: ".$nom_alu.". El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
        
        } else if ( $tipo_usuario == 'Profesor' ) {
        
          $des_log = 'El Profesor: '.$nombre_usuario." ".$accion." un ".$tipo_actividad." ( ".$nom_act." ), de la materia de ".$nom_mat.", del grupo ".$nom_gru.", en el programa: ".$nom_ram.". Del alumno: ".$nom_alu.". El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
    ...
    while( $fila = mysqli_fetch_assoc( $resultado ) ){
      ...
      if ( $fila['id_sub_hor6'] == NULL ) {
        ...
        $total++;
          ...
          // VALIDACION DE QUE PROFESOR SIGUE ACTIVO EN EL GRUPO
          $sqlValidacion = "
              SELECT *
              FROM sub_hor
              WHERE ( id_pro1 = '$id_usuario') AND (id_sub_hor = '$id_sub_hor') AND (est_sub_hor = 'Activo')
          ";
      ...
      if ( $fila['id_sub_hor6'] == NULL ) {
        ...
        $total++;
          ...
          $sqlValidacion = "
              SELECT *
              FROM sub_hor
              WHERE ( id_pro1 = '$id') AND (id_sub_hor = '$id_sub_hor') AND (est_sub_hor = 'Activo')
          ";
  ...
  function obtenerDatosUsuarioSalaServer( $id_sal, $tipo_usuario, $id_usuario ){
	...
	function obtenerTotalNotificacionesGrupoServer( $id, $id_sub_hor ){
		require( '../../includes/conexion.php' );
		...
		$sql = "
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor2 = '$id_sub_hor' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor3 = '$id_sub_hor' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor4 = '$id_sub_hor' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            ORDER BY fecha DESC
        "; 
	...
	function obtenerNavegacionGrupo( $id_sub_hor, $id ){
		require( '../includes/conexion.php' );
...
?>
		...
		<div class="fixed-action-btn" style="bottom: 45px; right: 24px;">
		    <a class="btn-floating btn-lg grey waves-effect waves-light" title="Acceso rápido">
		      	<i class="fas fa-chalkboard-teacher"></i>
		    </a>

		    <ul class="list-unstyled">
				<li>
					<a class="btn-floating green accent-4 waves-effect waves-light" title="Clases de <?php echo $filaMateria['nom_mat'].' '.$filaMateria['nom_gru']; ?> " href="clases_materia.php?id_sub_hor=<?php echo $filaMateria['id_sub_hor']; ?>">
				...
				<li>
					<a class="btn-floating red waves-effect waves-light" title="Listado de alumnos de <?php echo $filaMateria['nom_mat'].' '.$filaMateria['nom_gru']; ?> " href="alumnos_materia.php?id_sub_hor=<?php echo $filaMateria['id_sub_hor']; ?>">
				...
				<li>
					<a class="btn-floating amber accent-4 waves-effect waves-light" title="Calificar actividades de <?php echo $filaMateria['nom_mat'].' '.$filaMateria['nom_gru']; ?> " href="actividades_materia.php?id_sub_hor=<?php echo $filaMateria['id_sub_hor']; ?>">
				...
				<li>
					<a class="btn-floating grey darken-2 waves-effect waves-light" title="Video-clase de <?php echo $filaMateria['nom_mat'].' '.$filaMateria['nom_gru']; ?> " href="video_clase.php?id_sub_hor=<?php echo $filaMateria['id_sub_hor']; ?>">
...
<?php
	...
	function obtenerTotalPuntajeGrupoServer( $id_sub_hor ){
		require( '../../includes/conexion.php' );
	...
	function obtenerTotalPuntajeGrupo( $id_sub_hor ){
		require( '../includes/conexion.php' );
  ...
  function obtenerPorcentajeParticipacionActividad( $tipo, $identificador ){
    require( '../includes/conexion.php' );
	...
	function obtenerTotalNotificacionesActividadServer( $identificador_copia, $id ){
		...
		$sql = "
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_for_cop = '$identificador_copia' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            SELECT fec_cal_act AS fecha, nom_ent AS actividad, concat_ws(' ',nom_alu, app_alu, apm_alu) AS alumno, id_ent_cop AS identificador_copia, fot_alu AS foto, tip_ent AS tipo_actividad, nom_sub_hor AS clave, nom_mat AS nom_mat, id_alu_ram AS id_alu_ram, nom_gru AS nom_gru
            FROM cal_act
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_ent_cop = '$identificador_copia' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            SELECT fec_cal_act AS fecha, nom_exa AS actividad, concat_ws(' ',nom_alu, app_alu, apm_alu) AS alumno, id_exa_cop AS identificador_copia, fot_alu AS foto, tip_exa AS tipo_actividad, nom_sub_hor AS clave, nom_mat AS nom_mat, id_alu_ram AS id_alu_ram, nom_gru AS nom_gru
            FROM cal_act
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_exa_cop = '$identificador_copia' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            ORDER BY fecha DESC
        ";
	...
	function obtenerValidacionAlumnoActividadServer( $tipo_actividad, $identificador_copia, $id_alu_ram ){
		require( '../../includes/conexion.php' );
		...
		if ( $tipo_actividad == 'Foro' ) {
			...
			$sql = "
	            ...
	            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
	            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
	            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_alu_ram4 = '$id_alu_ram' ) AND ( id_for_cop = '$identificador_copia' )
			...
			$sql = "
	            ...
	            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
	            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
	            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_alu_ram4 = '$id_alu_ram' ) AND ( id_ent_cop = '$identificador_copia' )
			...
			$sql = "
	            ...
	            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
	            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
	            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_alu_ram4 = '$id_alu_ram' ) AND ( id_exa_cop = '$identificador_copia' )
	...
	function obtenerTotalNotificacionesBloqueGrupo( $id, $id_sub_hor, $id_blo ){
		require( '../includes/conexion.php' );
		...
		$sql = "
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor2 = '$id_sub_hor' ) AND ( id_blo4 = '$id_blo' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            SELECT fec_cal_act AS fecha, nom_ent AS actividad, concat_ws(' ',nom_alu, app_alu, apm_alu) AS alumno, id_ent_cop AS identificador_copia, fot_alu AS foto, tip_ent AS tipo_actividad, nom_sub_hor AS clave, nom_mat AS nom_mat, id_alu_ram AS id_alu_ram, nom_gru AS nom_gru
            FROM cal_act
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor3 = '$id_sub_hor' )AND ( id_blo5 = '$id_blo' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            UNION
            SELECT fec_cal_act AS fecha, nom_exa AS actividad, concat_ws(' ',nom_alu, app_alu, apm_alu) AS alumno, id_exa_cop AS identificador_copia, fot_alu AS foto, tip_exa AS tipo_actividad, nom_sub_hor AS clave, nom_mat AS nom_mat, id_alu_ram AS id_alu_ram, nom_gru AS nom_gru
            FROM cal_act
            ...
            INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
            INNER JOIN grupo ON grupo.id_gru = sub_hor.id_gru1
            WHERE (fec_cal_act IS NOT null ) AND ( pun_cal_act IS NULL ) AND ( id_pro1 = '$id' ) AND ( id_sub_hor4 = '$id_sub_hor' )AND ( id_blo6 = '$id_blo' )
            GROUP BY identificador_copia, identificador_copia, fecha, actividad, alumno, foto, tipo_actividad, clave, nom_mat, id_alu_ram, nom_gru
            ORDER BY fecha DESC
        "; 
	...
	function obtenerTotalNotificacionesGrupo( $id, $id_sub_hor ){
		require( '../includes/conexion.php' );
  ...
  function obtenerTotalActividadesBloqueServer( $id_blo ){
    require( '../../includes/conexion.php' );
  ...
  function obtenerValidacionAlumnosCalificadosGrupo( $id_sub_hor ){
    require( '../includes/conexion.php' );
    ...
    $sql = "
      ...
      INNER JOIN alumno ON alumno.id_alu = alu_ram.id_alu1
      INNER JOIN materia ON materia.id_mat = sub_hor.id_mat1
      WHERE id_sub_hor = '$id_sub_hor' AND est_sub_hor = 'Activo' AND est_alu_hor = 'Activo'
    ";
    ...
    while( $fila = mysqli_fetch_assoc( $resultado ) ){
      ...
      $filaCalificacion = mysqli_fetch_assoc( $resultadoCalificacion );

      if ( ( $filaCalificacion['ext_cal'] == NULL ) && ( $filaCalificacion['fin_cal'] == NULL ) ) {
        $validador = 'verdadero';
	...
	function obtenerTotalAlumnosGrupo( $id_sub_hor ){
 		require( '../includes/conexion.php' );
 	...
 	function obtenerTotalAlumnosGrupoServer( $id_sub_hor ){
 		require( '../../includes/conexion.php' );
	...
	function obtenerEnterosMasterServer( $id_sub_hor, $fechaInicio, $fechaFin ){
		require('../../includes/conexion.php');
		...
		if ( $resultado ) {
			...
			$inicioEntero = strtotime( $fechaInicio ) - strtotime( $fila['ini_cic'] );

			$inicio = round( $inicioEntero / ( 60 * 60 * 24) );

			$finEntero = strtotime( $fechaFin ) - strtotime( $fila['ini_cic'] );

			$fin = round( $finEntero / ( 60 * 60 * 24 ) );
	...
	function obtenerDatosActividadGrupoServer( $identificador, $tipo, $formato ){
		require('../../includes/conexion.php');
		...
		if ( $tipo == 'Foro' ) {
			...
			$sql = "
				SELECT id_for AS identificador, nom_for AS titulo, des_for AS descripcion, fec_for AS fecha, tip_for AS tipo, pun_for AS puntaje, ini_for_cop AS inicio, fin_for_cop AS fin
				FROM foro_copia
				INNER JOIN foro ON foro.id_for = foro_copia.id_for1
			...
			$sql = "
				SELECT id_ent AS identificador, nom_ent AS titulo, des_ent AS descripcion, fec_ent AS fecha, tip_ent AS tipo, pun_ent AS puntaje, ini_ent_cop AS inicio, fin_ent_cop AS fin
				FROM entregable_copia
				INNER JOIN entregable ON entregable.id_ent = entregable_copia.id_ent1
			...
			$sql = "
				SELECT id_exa AS identificador, nom_exa AS titulo, des_exa AS descripcion, fec_exa AS fecha, tip_exa AS tipo, pun_exa AS puntaje, ini_exa_cop AS inicio, fin_exa_cop AS fin, dur_exa AS duracion
				FROM examen_copia
				INNER JOIN examen ON examen.id_exa = examen_copia.id_exa1
	...
	function contadorRecursosPracticosServer( $id_blo, $id_sub_hor ){
		require('../../includes/conexion.php');
		...
		$sql = "
			...
			UNION
			SELECT id_exa AS identificador, nom_exa AS titulo, des_exa AS descripcion, fec_exa AS fecha, tip_exa AS tipo, pun_exa AS puntaje, ini_exa_cop AS inicio, fin_exa_cop AS fin
			FROM examen_copia
			INNER JOIN examen ON examen.id_exa = examen_copia.id_exa1
  ...
  // function function obtenerGruposBloqueServer( $id_blo ){
  //   require('../../includes/conexion.php');
  ...
  // }
	...
	function obtenerDescripcionNodoProgramaLogServer( $tipo_quien, $quien, $que, $tipo_que, $a_que, $donde ){
		...
		if ( $tipo_quien == 'Admin' ) {
          ...
          $des_log = 'El Administrador: '.$quien." ".$que." un(a) ".$tipo_que." ( ".$a_que." ) en el programa: ".$donde.". El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
            ...
            $des_log = 'El Profesor: '.$quien." ".$que." un(a) ".$tipo_que." ( ".$a_que." ) en el programa: ".$donde.". El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
	...
	function obtenerDescripcionAlumnoCalificacionLogServer( $tipoUsuario, $quien, $que, $materia, $calificacion, $alumno, $programa ){
		...
		if ( $tipoUsuario == 'Admin' ) {
            ...
            $des_log = 'El Profesor: '.$quien." ".$que." la calificación ( ".$materia." con ".$calificacion." ), de ".$alumno." en ( ".$programa." ). El dia ".fechaHoraFormateadaCompactaServer( $fechaHoy ).".";
...
//FUNCION PARA SABER DIFERENCIA DE DIAS...LO DA COMO UN STRING
  ...
  function generadorFaltas($id_alu_ram){
    ...
    // GENERADOR DESCUENTOS O RECARGOS POR PAGO
	function generadorDescuentosRecargosPago($id_pag, $whatsappPlantel, $smsPlantel, $emailPlantel, $client){
		require('../../includes/conexion.php');
                           ...
                           if( $resultadoBuscaAbono ){
                                ...
                                if ( ($totalBuscaAbono > 0) || ($totalCondonacion > 0) ) {
                                     ...
                                     $sqlTotalAbonado = "
                                          ...
                                          WHERE id_pag1 = '$id_pag'
                                          UNION
                                          SELECT SUM(mon_con_pag) AS totalAbonado
                                     ...
                                     if ( $resultadoTotalAbonado ) {
                                          ...
                                          $nuevoMonto = $filaPago['mon_ori_pag'] - $totalAbonado;
                                     }
                                     ...
                                     $nuevoMonto = $filaPago['mon_ori_pag'];
                           ...
                           if ( $resultadoValidacionHistorial ) {
                                ...
                                if ( ($validacionHistorial == 0) ) {
                                     ...
                                     if ( $filaPago['mon_pag'] < $filaPago['mon_ori_pag'] ) {
                                          ...
                                          $con_his_pag = "Eliminación de descuento de: ".$descuento.", fijando el adeudo en su monto original, $".$filaPago['mon_ori_pag']." por concepto: ".$filaPago['con_pag'].".  Si se realiza antes del ".fechaFormateadaCompacta($filaPago['fin_pag']);
                                     }else{
                           ...
                           if( $resultadoBuscaAbono ){
                                ...
                                if ( ($totalBuscaAbono > 0) || ($totalCondonacion > 0) ) {
                                     ...
                                     // CASO PORCENTUAL
                                          ...
                                          $recargo = ($filaPago['mon_ori_pag'] - $totalAbonado)*( ( ( $filaPago['car_pag']/100 )/365 )*$diferenciaDias )  ;
                                          $nuevoMonto = $filaPago['mon_ori_pag'] - $totalAbonado + $recargo;

                                     // FIN CASO PORCENTUAL
                                     }else if( $filaPago['tip2_pag'] == 'Monetario' ){
                                     // CASO MONETARIO
                                          $recargo = ($filaPago['car_pag']*$diferenciaDias);
                                          $nuevoMonto = $filaPago['mon_ori_pag'] - $totalAbonado + $recargo;
                                     // FIN CASO MONETARIO
                                     }
                                ...
                                // CASO PORCENTUAL
                                     ...
                                     $recargo = $filaPago['mon_ori_pag']*( ( ( $filaPago['car_pag']/100 )/365 )*$diferenciaDias )  ;
                                     $nuevoMonto = $filaPago['mon_ori_pag'] + $recargo;
                      ...
                      if ( $resultadoUpdatePago ) {
                           ...
                           if ( $resultadoValidacionHistorial ) {
                                ...
                                if ( ($validacionHistorial == 0) || ($validacionHistorial == 0) && ($diferenciaDias%3 == 0) ) {
                                     ...
                                     $con_his_pag = "Se generó recargo de $".round($recargo, 2)." al monto original de $".$filaPago['mon_ori_pag'].". Nuevo adeudo de $".round($nuevoMonto, 2);

                                     $men_his_pag = "Se generó recargo de $".round($recargo, 2)." al monto original de $".$filaPago['mon_ori_pag'].". Nuevo adeudo de $".round($nuevoMonto, 2);
  ...
  // FIN VALIDADOR SI EXISTEN FECHAS
    ...
    // FIN GENERADOR DESCUENTOS O RECARGOS POR PAGO
    ...
    function obtenerConsultaHorarioServer( $id_gru ){
    ...
    // DATOS MATERIA
    $nom_mat = $filaHorarioDatos['nom_mat'];
...
?>
    ...
    <div class="row">
      <div class="col-md-12">
            ...
            <br>
            <label for="">
              Consulta de horario para <?php echo $nom_gru; ?>
            </label>
