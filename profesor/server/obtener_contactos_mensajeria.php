<?php
  ...
  $sqlSalas = "
    ...
    WHERE usu_usu_sal = '$id' AND tip_usu_sal = '$tipo'
    
  ";

  if ( $_POST['id_sub_hor'] != 'falso' ) {
      
    $sqlSalas .= 'ORDER BY id_sub_hor6 = '.$_POST['id_sub_hor'].' DESC, fec_men_sal DESC ';  

  } else {

    $sqlSalas .= 'ORDER BY fec_men_sal DESC ';

  }
   

  $sqlSalas .= "LIMIT $inicio, $limite";
  // echo $sqlSalas;
  ...
  while ( $filaSalas = mysqli_fetch_assoc( $resultadoSalas ) ) {
    ...
    $sqlUltimoMensaje = "
      SELECT hor_men,tip_men,use_men,men_men FROM mensaje WHERE id_sal4 = '$id_sal6' ORDER BY hor_men DESC LIMIT 1
    ";
...
?>
    ...
    <div class="chat-list-item d-flex flex-row w-100 p-2 border-bottom unread seleccionSala waves-effect <?php echo ( $_POST['id_sub_hor'] != 'falso' && $i == 1 )? 'animated pulse delay-2s': ''; ?>" id_sal="<?php echo $id_sal6; ?>" sala_nombre="<?php echo $sala_nombre; ?>" sala_foto="<?php echo obtenerValidacionFotoUsuario( $sala_foto ); ?>" id_sub_hor="<?php echo $id_sub_hor; ?>">
      
      <?php  
        if ( $filaSalas['id_sub_hor6'] == NULL ) {
      ?>

          <img src="<?php echo obtenerValidacionFotoUsuario( $sala_foto ); ?>" class="img-fluid rounded-circle mr-2" style="height:50px; position: relative; border: 2px solid black;">

      <?php
        } else {
      ?>

          <img src="../img/<?php echo $sala_foto; ?>" class="img-fluid rounded-circle mr-2" style="height:50px; position: relative; border: 2px solid black;">
      ...
      <div class="w-50">
        ...
        <div class="badge badge-warning badge-pill small font-weight-normal" ><?php echo $sala_tipo; ?></div>
        ...
        ?>
            <div class="badge badge-primary badge-pill small font-weight-normal"><?php echo $filaSalas['nom_sal']; ?></div>
        <?php
          }
        ?>
        <div class="name"><?php echo $sala_nombre; ?></div>

        <div class="small last-message"><span id="seleccionSalaContactoUltimoMensaje<?php echo $id_sal6; ?>"><?php echo $mensaje_ultimo_contacto; ?></span> dice: <span id="seleccionSalaMensaje<?php echo $id_sal6; ?>"><?php echo comprimirTextoVariable( $mensaje_ultimo, 10 ); ?></span>
        </div>
      ...
      <div class="flex-grow-1 text-right">
        ...
        <div id="seleccionSalaNotificacion<?php echo $id_sal6; ?>">
          <?php
            $sqlConteo = "
              SELECT obtener_notificaciones_sala( '$id_sal6', '$id', '$tipo' ) AS notificaciones_sala
            ";

            $notificaciones_sala = obtener_datos_consulta( $db, $sqlConteo )['datos']['notificaciones_sala'];

            if ( $notificaciones_sala > 0 ) {
              echo '<div class="badge badge-success badge-pill small" id="unread-count">'.$notificaciones_sala.'</div>';
            }
...
<script>
  $('.seleccionSala').on('click', function(event) {
    ...
    // alert(id_sub_hor);
    $('.btn_accion_mensajeria').css('display', '');

    if ( id_sub_hor == 'Falso' ) {
      $('#btn_eliminacion_sala').css('display', '');
    } else {
      $('#btn_eliminacion_sala').css('display', 'none');
    }
  ...
  function obtener_datos_sala( id_sal ){
    ...
    $.ajax({
      ...
      success: function( datos ){
        ...
        // console.log( datos.notificacion );
        console.log( datos.mensaje.men_men );
        $('#seleccionSalaMensaje'+id_sal).text( datos.mensaje.men_men.substring(0, 10) );
        ...
        if ( datos.notificacion == 0 ) {
          ...
          $('#seleccionSalaNotificacion'+id_sal).html( '<div class="badge font-weight-normal badge-success badge-pill small" id="unread-count">'+datos.notificacion+'</div>' );
       ...
       for( var i = 0; i < nombres.length; i++ ){
           ...
           item = $(nombres[i]).attr('sala_nombre').toLowerCase();
            ...
            for(var x = 0; x < item.length; x++ ){
                ...
                // console.log(item.indexOf( palabra ));
                // console.log( nombres );
