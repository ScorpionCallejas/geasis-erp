...
// 🔥 FUNCIÓN PARA OBTENER LA CUENTA STRIPE DEL PLANTEL
function obtener_clave_sucursal($id_pla) {
    $cuentas_stripe = array(
        2 => 'acct_1ROncSDX0jVgLrbV',
        3 => 'acct_1RP98QKDoALPSXl5',
        6 => 'acct_1RPBT2CrnTxsEL2D',
        8 => 'acct_1RP7YXIorZenyljq',
        9 => 'acct_1RP7gYCcfhkAfik9',
        13 => 'acct_1RP7xEBggmXH7CEA'
    );
    
    if (isset($cuentas_stripe[$id_pla])) {
        return $cuentas_stripe[$id_pla];
    }
    
    return 'acct_1RHXUrAr8nVhZRNR';
}

// 🔥 OBTENER LA CUENTA STRIPE DEL PLANTEL ACTUAL
$cuenta_stripe = obtener_clave_sucursal($plantel);
...
const alumnoData = {
    ...
    cuenta_stripe: '<?php echo $cuenta_stripe; ?>'
};

// 🔥 INICIALIZAR STRIPE CON CUENTA CONECTADA
const stripe = Stripe(STRIPE_PUBLIC_KEY, {
    stripeAccount: alumnoData.cuenta_stripe  // 🔥 PASAMOS LA CUENTA AQUÍ
});

log('info', '🔥 Stripe inicializado con cuenta conectada', {
    public_key: STRIPE_PUBLIC_KEY.substring(0, 20) + '...',
    cuenta_stripe: alumnoData.cuenta_stripe
});
...
$(document).ready(function() {
    log('info', '✅ Página de pagos cargada', {
    ...
    $('#modalPagoStripe').on('hidden.bs.modal', function() {
        if (elements && typeof elements.destroy === 'function') { 
            elements.destroy(); 
        }
        elements = null;
    ...
    $('#modalActivarDom').on('hidden.bs.modal', function() {
        if (setupElements && typeof setupElements.destroy === 'function') { 
...
async function cargarEstadoDomiciliacion() {
    try {
        ...
        log('error', '❌ Error en cargarEstadoDomiciliacion()', error);
...
async function inicializarSetup() {
    try {
        ...
        window.setupCustomerId = data.customer;
        window.setupIntentId = data.setupIntentId;
        console.log('🔥 CUSTOMER GUARDADO:', window.setupCustomerId);
...
document.getElementById('setup-form').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        if (error) {
            ...
            console.log('🔥 SETUP INTENT COMPLETO:', setupIntent);
            ...
            await vincularTarjeta(setupIntent);
...
// ========================================
// 🔗 VINCULAR TARJETA EN BD
// ========================================
async function vincularTarjeta(setupIntent) {
    try {
        log('info', '💳 Vinculando tarjeta al backend');
        
        const setupIntentId = setupIntent.id || window.setupIntentId;
        const customerId = setupIntent.customer || window.setupCustomerId;
        
        if (!setupIntentId) {
            log('error', '❌ No hay setupIntentId disponible');
            throw new Error('No se pudo obtener el ID del setup intent');
        }
        
        if (!customerId) {
            log('error', '❌ No hay customer_id disponible', {
                setupIntent: setupIntent,
                setupCustomerId: window.setupCustomerId
            });
            throw new Error('No se pudo obtener el ID del cliente');
        }
        
        log('info', '✅ Setup Intent ID confirmado', setupIntentId);
        log('info', '✅ Customer ID confirmado', customerId);
        
        log('info', '🔍 Solicitando datos de tarjeta al backend Node.js');
        
        const pmResponse = await fetch(`${API_BASE_URL}/obtener-payment-method`, {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({ 
        ...
        if (!pmData.success) {
            log('error', '❌ Error al obtener datos de tarjeta', pmData);
            throw new Error(pmData.message || 'No se pudieron obtener los datos de la tarjeta');
        }
        
        const paymentMethodId = pmData.payment_method_id;
        
        log('info', '📇 Datos de la tarjeta obtenidos', {
            payment_method_id: paymentMethodId,
            last4: pmData.data.card.last4,
            brand: pmData.data.card.brand,
            exp_month: pmData.data.card.exp_month,
            exp_year: pmData.data.card.exp_year
        });
        
        const formData = new FormData();
        formData.append('accion', 'vincular_tarjeta');
        formData.append('id_alu_ram', alumnoData.id_alu_ram);
        formData.append('customer_id', customerId);
        formData.append('payment_method_id', paymentMethodId);
        formData.append('last_4', pmData.data.card.last4);
        formData.append('brand', pmData.data.card.brand);
        formData.append('exp_month', pmData.data.card.exp_month);
        formData.append('exp_year', pmData.data.card.exp_year);
        
        log('info', '📤 Enviando request al backend PHP con datos de tarjeta');
        ...
        if (!contentType || !contentType.includes('application/json')) {
            log('error', '❌ La respuesta no es JSON', {
                contentType: contentType,
                status: response.status
            });
            throw new Error('El servidor devolvió un error PHP. Verifica los logs del servidor.');
        }
        ...
        if (!data.success) {
            log('error', '❌ Error del backend', {
                message: data.message,
                fullResponse: data
            });
            
            throw new Error(data.message || 'Error desconocido al vincular tarjeta');
        }
        
        if (!data.data || !data.data.last_4 || !data.data.brand) {
            log('error', '❌ Falta información de la tarjeta en la respuesta', {
                data_recibida: data.data
            });
            throw new Error('No se obtuvieron los datos completos de la tarjeta');
        }
        ...
        log('error', '❌ Error en vincularTarjeta()', {
            error: error.message,
            stack: error.stack
        });
        ...
        if (mensajeUsuario.includes('customer_id inválido')) {
            mensajeUsuario = 'Error: ID de cliente inválido. Intenta cerrar sesión y volver a intentar.';
        } else if (mensajeUsuario.includes('payment_method_id inválido')) {
            mensajeUsuario = 'Error: Método de pago inválido. Intenta con otra tarjeta.';
        } else if (mensajeUsuario.includes('Payment method inválido')) {
            mensajeUsuario = 'La tarjeta no pudo ser validada. Verifica los datos e intenta nuevamente.';
        }
        
        showMessage(mensajeUsuario, 'danger', 'setup-message');
        setLoading(false, 'setup');
        ...
        Swal.fire({
            ...
            text: mensajeUsuario,
            footer: DEBUG_MODE ? `<small class="text-muted">Error técnico: ${error.message}</small>` : ''
        });
    }
}
...
async function cancelarDomiciliacion() {
    try {
        log('info', '🗑️ Cancelando domiciliación');
        
        const formData = new FormData();
        formData.append('accion', 'cancelar_domiciliacion');
        formData.append('id_alu_ram', alumnoData.id_alu_ram);
