	...
	<table id="myTable" class="table table-hover table-bordered table-sm" cellspacing="0" width="100%">
		...
		?>
			<tr>
				...
				<td><?php echo $filaDocumentos['est_doc_ram']; ?></td>
	      ...
	      <div class="modal-body mx-3">
			...
			<div class="md-form">
				...
				<div class="form-check">
				  	...
				  	<input type="checkbox" class="form-check-input radioReporte" id="checkbox_est_doc_ram2" name="checkbox_est_doc_ram2">
				  	<label class="form-check-label letraPequena" for="checkbox_est_doc_ram2">Permitir subir archivo</label>
...
<script>
	...
	$('#agregarDocumentoFormulario').on('submit', function(event) {
		...
		if ( $('#checkbox_est_doc_ram')[0].checked == true ) {
		  // console.log("checkeado");
		  	
			agregarDocumentoFormulario.append('est_doc_ram', 'Activo'); 

		
		} else { 
		  
			// console.log('unchecked');
			agregarDocumentoFormulario.append('est_doc_ram', 'Inactivo');
	...
	$('.edicion').on('click', function(){
		...
		$.ajax({
			...
			success: function(datos){
				...
				if ( datos.est_doc_ram == 'Activo' ) {
				  // console.log("checkeado");
				  	
					$('#checkbox_est_doc_ram2').prop({ checked: true });

				
				} else { 
				  
					// console.log('unchecked');
					$('#checkbox_est_doc_ram2').prop({ checked: false });
				...
				$('#editarDocumentoFormulario').on('submit', function(event) {
					...
					if ( $('#checkbox_est_doc_ram2')[0].checked == true ) {
					  // console.log("checkeado");
					  	
						editarDocumentoFormulario.append('est_doc_ram', 'Activo'); 
