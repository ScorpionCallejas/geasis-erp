<?php
  // Función para calcular fechas por semana
	function calcularFechaSemana($fecha_inicio, $semana) {
		$fecha = new DateTime($fecha_inicio);
		$fecha->add(new DateInterval('P' . ($semana - 1) . 'W'));
		return $fecha->format('d/m/Y');
	}
	
	// ========== GENERACIÓN AUTOMÁTICA DE GASTOS FIJOS MENSUALES ==========
  function generar_gastos_fijos_mes_actual($db, $cadena, $responsable) {
      
    $mes_actual = (int)date('n');  // 1-12
    $anio_actual = (int)date('Y');
    
    // ========== PASO 1: VERIFICAR SI YA SE GENERÓ ESTE MES ==========
    $sql_check = "
        SELECT id FROM control_gastos_fijos 
    ...
    if(!$resultado_check) {
        // Error en la consulta, no hacer nada
        return;
    }
    
    if(mysqli_num_rows($resultado_check) > 0) {
        // YA SE GENERÓ ESTE MES, NO HACER NADA
        return;
    }
    
    // ========== PASO 2: CALCULAR MES ANTERIOR ==========
    $mes_anterior = $mes_actual - 1;
    $anio_buscar = $anio_actual;
    ...
    $primer_dia_mes_anterior = "$anio_buscar-" . str_pad($mes_anterior, 2, '0', STR_PAD_LEFT) . "-01";
    $ultimo_dia_mes_anterior = date("Y-m-t", strtotime($primer_dia_mes_anterior));
    
    // ========== PASO 3: BUSCAR GASTOS FIJOS DEL MES ANTERIOR ==========
    $sql_fijos = "
        SELECT * FROM egreso 
        WHERE rec_egr = 'Gasto fijo' 
        AND fec_egr BETWEEN '$primer_dia_mes_anterior' AND '$ultimo_dia_mes_anterior'
    ";
    
    $resultado_fijos = mysqli_query($db, $sql_fijos);
    ...
    // ========== PASO 4: CLONAR CADA GASTO FIJO ==========
    while($gasto = mysqli_fetch_assoc($resultado_fijos)) {
        
        // Fecha nueva: día 1 del mes actual
        $fecha_nueva = date('Y-m-d');
        
        // Escapar valores
        $id_pla13 = mysqli_real_escape_string($db, $gasto['id_pla13']);
        $con_egr = mysqli_real_escape_string($db, $gasto['con_egr']);
        $mon_egr = floatval($gasto['mon_egr']);
        $id_cat_egr = intval($gasto['id_cat_egr']);
        $for_egr = mysqli_real_escape_string($db, $gasto['for_egr']);
        $obs_egr = mysqli_real_escape_string($db, $gasto['obs_egr']);
        $ori_egr = mysqli_real_escape_string($db, $gasto['ori_egr']);
        
        // Agregar nota de auto-generado en observaciones
        $obs_nueva = "AUTO-GENERADO DEL MES ANTERIOR";
        if(!empty($obs_egr)) {
            $obs_nueva .= " | " . $obs_egr;
        }
        ...
        $sql_insert = "
            INSERT INTO egreso (
                id_pla13, con_egr, mon_egr, tip_egr, fec_egr, 
                res_egr, id_cat_egr, for_egr, obs_egr, 
    ...
    // ========== PASO 5: REGISTRAR EN TABLA DE CONTROL ==========
    if($total_generados > 0) {
        $sql_marcar = "
            INSERT INTO control_gastos_fijos (mes, anio, fecha_generacion, generado_por, total_generados)
            VALUES ($mes_actual, $anio_actual, NOW(), '$responsable', $total_generados)
        ";
        mysqli_query($db, $sql_marcar);
    }
	...
	// ......
	function enviar_correo_cambio_password_ejecutivo($id_eje, $db) {
    ...
    // ============================================================================
    $id_eje_escaped = mysqli_real_escape_string($db, $id_eje);
    ...
    $sql = "
        SELECT 
            ...
            tel_eje
        FROM ejecutivo 
        WHERE id_eje = '$id_eje_escaped'
    ";
    
    $resultado = mysqli_query($db, $sql);
    
    if(!$resultado || mysqli_num_rows($resultado) == 0) {
        error_log("Error: Ejecutivo ID $id_eje no encontrado");
        return array(
            'success' => false,
    ...
    $ejecutivo = mysqli_fetch_assoc($resultado);
    
    // ============================================================================
    // VALIDAR QUE TENGA CORREO PERSONAL
    // ============================================================================
    if(empty($ejecutivo['cor2_eje'])) {
        error_log("Error: El ejecutivo ID $id_eje no tiene correo personal");
        return array(
            'success' => false,
            'error' => 'No se encontró correo personal'
        );
    }
    ...
    try {
        ...
        // ====================================================================
        $mail->setFrom('contacto@ahjende.com', 'AHJ ENDE - Seguridad');
        $mail->addAddress($ejecutivo['cor2_eje']); // Correo personal
        
        // ====================================================================
        // ASUNTO
        // ====================================================================
        $mail->Subject = '🔐 Confirmación de Cambio de Contraseña - AHJ ENDE';
        
        // ====================================================================
        // COLORES DEL TEMPLATE
        // ====================================================================
        $color_principal = '#0588a6';  // Azul tenue
        $color_secundario = '#304357'; // Azul oscuro
        $color_verde = '#4caf50';      // Verde
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>Hola <strong style="color: '.$color_verde.';">'.htmlspecialchars($ejecutivo['nom_eje']).'</strong>,</p>
                    
                    <div class="success-confirmation">
                        <p style="margin: 0; font-weight: bold; font-size: 14px; color: '.$color_verde.';">
                            <span class="check-icon">✅</span>Tu contraseña ha sido actualizada correctamente
                        </p>
                        <p style="margin: 5px 0 0 0; font-size: 12px;">El cambio se realizó el día '.date('d/m/Y').' a las '.date('H:i:s').'</p>
                    </div>
                    
                    <p>Te confirmamos que tu contraseña ha sido cambiada exitosamente. A continuación, te compartimos tus credenciales de acceso actualizadas:</p>
                    
                    <div class="datos">
                        <h3 style="font-size: 15px; margin-top: 0; color: '.$color_verde.';">🔑 Datos de acceso al sistema:</h3>
                        <p class="credentials"><strong>Usuario:</strong> <span class="highlight">'.htmlspecialchars($ejecutivo['cor_eje']).'</span></p>
                        <p class="credentials"><strong>Nueva contraseña:</strong> <span class="highlight">'.htmlspecialchars($ejecutivo['pas_eje']).'</span></p>
                        <p class="credentials"><strong>Rango:</strong> <span class="highlight">'.htmlspecialchars($ejecutivo['ran_eje']).'</span></p>
                        <p class="credentials"><strong>Plataforma:</strong> <span class="highlight"><a href="https://plataforma.ahjende.com/ejecutivo" style="color: '.$color_principal.';">https://plataforma.ahjende.com/ejecutivo</a></span></p>
                    </div>
                    
                    <div class="security-note">
                        <p style="font-size: 13px; color: #856404; margin: 0; line-height: 1.6;">
                            <strong><i>🛡️ Nota de seguridad:</i></strong><br>
                            • Tu contraseña vencerá en <strong>45 días</strong><br>
                            • Recibirás un aviso <strong>7 días antes</strong> del vencimiento<br>
                            • Recuerda mantener tu información segura y no compartir tus credenciales
                        </p>
                    </div>
                    
                    <div class="info-box">
                        <p style="margin: 0 0 8px 0;"><strong>📋 Información de tu cuenta:</strong></p>
                        <p style="margin: 3px 0;"><strong>Nombre:</strong> '.htmlspecialchars($ejecutivo['nom_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Correo personal:</strong> '.htmlspecialchars($ejecutivo['cor2_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Teléfono:</strong> '.htmlspecialchars($ejecutivo['tel_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Fecha de actualización:</strong> '.date('d/m/Y H:i:s').'</p>
                    </div>
                    
                    <p style="font-size: 13px; color: #666; background: #e3f2fd; padding: 12px; border-radius: 6px; border-left: 3px solid '.$color_principal.';">
                        <strong>❓ ¿No fuiste tú?</strong><br>
                        Si no realizaste este cambio, contacta inmediatamente al equipo de soporte para proteger tu cuenta.
                    </p>
                </div>
                
                <div class="footer">
                    <p style="margin: 0 0 5px 0;"><strong>© '.date('Y').' AHJ ENDE</strong></p>
                    <p style="margin: 0;">Escuela de Negocios y Desarrollo Empresarial</p>
                    <p class="privacidad">
                        <a href="https://ahjende.com/aviso-de-privacidad" style="color: '.$color_principal.'; text-decoration: none;">Aviso de Privacidad</a>
                    </p>
                </div>
            </div>
        </body>
        ...
        // ====================================================================
        $mail->AltBody = 
            "CAMBIO DE CONTRASEÑA EXITOSO - AHJ ENDE\n\n" .
            "Hola {$ejecutivo['nom_eje']},\n\n" .
            "Tu contraseña ha sido actualizada correctamente el día " . date('d/m/Y') . " a las " . date('H:i:s') . ".\n\n" .
            "Datos de acceso al sistema:\n" .
            "- Usuario: {$ejecutivo['cor_eje']}\n" .
            "- Nueva contraseña: {$ejecutivo['pas_eje']}\n" .
            "- Rango: {$ejecutivo['ran_eje']}\n" .
            "- Plataforma: https://plataforma.ahjende.com/ejecutivo\n\n" .
            "NOTA DE SEGURIDAD:\n" .
            "• Tu contraseña vencerá en 45 días\n" .
            "• Recibirás un aviso 7 días antes del vencimiento\n" .
            "• Recuerda mantener tu información segura\n\n" .
            "Información de tu cuenta:\n" .
            "- Nombre: {$ejecutivo['nom_eje']}\n" .
            "- Correo personal: {$ejecutivo['cor2_eje']}\n" .
            "- Teléfono: {$ejecutivo['tel_eje']}\n" .
            "- Fecha de actualización: " . date('d/m/Y H:i:s') . "\n\n" .
            "¿No fuiste tú?\n" .
            "Si no realizaste este cambio, contacta inmediatamente al equipo de soporte.\n\n" .
            "© " . date('Y') . " AHJ ENDE - Todos los derechos reservados\n" .
            "Aviso de Privacidad: https://ahjende.com/aviso-de-privacidad";
        ...
        // ERROR AL ENVIAR
        // ====================================================================
        error_log("Error al enviar correo de confirmación a {$ejecutivo['cor2_eje']}: " . $mail->ErrorInfo);
	...
	function enviar_codigo_validacion_ejecutivo($id_eje, $cor2_eje, $db) {
    
    // ========================================================================
    // 1. VALIDAR QUE EL EJECUTIVO EXISTA
    // ========================================================================
    $id_eje_escaped = mysqli_real_escape_string($db, $id_eje);
    
    $sql = "
        SELECT nom_eje, ran_eje, cor_eje
        FROM ejecutivo 
        WHERE id_eje = '$id_eje_escaped'
    ";
    ...
    // ========================================================================
    // 2. VALIDAR QUE TENGA CORREO PERSONAL
    // ========================================================================
    if(empty($cor2_eje)) {
        return array(
            'success' => false,
            'error' => 'No se proporcionó correo personal'
        );
    }
    
    // ========================================================================
    // 3. GENERAR CÓDIGO ALEATORIO DE 4 DÍGITOS
    // ========================================================================
    $codigo = str_pad(rand(0, 9999), 4, '0', STR_PAD_LEFT);
    
    // ========================================================================
    // 4. CALCULAR TIMESTAMP DE EXPIRACIÓN (3 minutos = 180 segundos)
    // ========================================================================
    $expiracion = time() + 180;
    
    // ========================================================================
    // 5. PREPARAR PHPMAILER
    // ========================================================================
    require_once(__DIR__.'/../../vendor/PHPMailer-master/src/PHPMailer.php');
    require_once(__DIR__.'/../../vendor/PHPMailer-master/src/Exception.php');
    require_once(__DIR__.'/../../vendor/PHPMailer-master/src/SMTP.php');
    ...
    try {
        ...
        <head>
            ...
            <style>
                ...
                .codigo-box {
                    background: #f0f0f0;
                    border: 3px dashed '.$color_secundario.';
                    border-radius: 12px;
                    padding: 35px;
                    text-align: center;
                    margin: 30px 0;
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    ...
                    <p>Hola <strong class="highlight">'.htmlspecialchars($datos['nom_eje']).'</strong>,</p>
                    
                    <p>Has solicitado cambiar tu contraseña en el sistema <strong>AHJ ENDE Ejecutivo</strong>. Para continuar con el proceso de forma segura, utiliza el siguiente código de verificación:</p>
                    ...
                    <div class="expiracion">
                        <p>
                            <strong>⏱️ IMPORTANTE:</strong> Este código expirará en <strong>3 minutos</strong> (180 segundos).
                        </p>
                    </div>
                    
                    <div class="info-box">
                        <p style="margin: 0 0 8px 0;"><strong>📋 Detalles de la solicitud:</strong></p>
                        <p style="margin: 3px 0;"><strong>Nombre:</strong> '.htmlspecialchars($datos['nom_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Rango:</strong> '.htmlspecialchars($datos['ran_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Usuario:</strong> '.htmlspecialchars($datos['cor_eje']).'</p>
                        <p style="margin: 3px 0;"><strong>Correo verificado:</strong> '.htmlspecialchars($cor2_eje).'</p>
                        <p style="margin: 3px 0;"><strong>Fecha:</strong> '.date('d/m/Y H:i:s').'</p>
                    </div>
                    
                    <p style="font-size: 13px; color: #666; background: #e3f2fd; padding: 12px; border-radius: 6px; border-left: 3px solid '.$color_principal.';">
                        <strong>🛡️ Nota de seguridad:</strong> Si no solicitaste este cambio, ignora este mensaje. Tu contraseña permanecerá sin cambios y nadie podrá acceder a tu cuenta.
                    </p>
                </div>
        ...
        // ====================================================================
        $mail->AltBody = 
            "CÓDIGO DE VERIFICACIÓN - AHJ ENDE\n\n" .
            "Hola {$datos['nom_eje']},\n\n" .
            "Has solicitado cambiar tu contraseña en el sistema AHJ ENDE Ejecutivo.\n\n" .
            "Tu código de verificación es:\n\n" .
            "    {$codigo}\n\n" .
            "IMPORTANTE: Este código expirará en 3 minutos.\n\n" .
            "Detalles de la solicitud:\n" .
            "- Nombre: {$datos['nom_eje']}\n" .
            "- Rango: {$datos['ran_eje']}\n" .
            "- Usuario: {$datos['cor_eje']}\n" .
            "- Correo verificado: {$cor2_eje}\n" .
            "- Fecha: " . date('d/m/Y H:i:s') . "\n\n" .
            "Nota de seguridad: Si no solicitaste este cambio, ignora este mensaje.\n\n" .
            "© " . date('Y') . " AHJ ENDE - Todos los derechos reservados";
        ...
        return array(
            ...
            'nombre_ejecutivo' => $datos['nom_eje']
        );
        ...
        // ERROR AL ENVIAR
        // ====================================================================
        error_log("Error al enviar código de verificación a {$cor2_eje}: " . $mail->ErrorInfo);
  ...
  function enviar_correo_alumno_prueba($id_alu_ram, $db) {
    ...
    if (empty($alumno['cor1_alu'])) {
      ...
      return [
        'success' => false,
        'mensaje' => 'El alumno no tiene correo electrónico',
        'correo' => null
      ];
    }
    ...
    try {
      ...
      <head>
        <meta charset="UTF-8">
        <meta name="viewport" content="width=device-width, initial-scale=1.0">
        <title>✉️ Validación de Correo - ' . htmlspecialchars($alumno['nom_ram']) . '</title>
        <style>
          body {
            font-family: Arial, sans-serif;
            line-height: 1.6;
      ...
      <body>
        <div class="container">
          ...
          <div class="content">
            <h1>Validación de Correo Electrónico, LÍDER <span class="lider">' . htmlspecialchars($alumno['nom_alu']) . '</span> ✉️</h1>
            
            <p>✨ Este es un correo de prueba para verificar que tu dirección de correo electrónico está configurada correctamente en nuestro sistema.</p>
            
            <div class="test-confirmation">
              <p style="margin: 0; font-weight: bold; font-size: 14px; color: '.$color_verde.';">
                <span class="check-icon">✅</span>Tu correo ha sido validado exitosamente
              </p>
              <p style="margin: 5px 0 0 0; font-size: 12px;">Si recibes este mensaje, tu correo está funcionando correctamente.</p>
            </div>
            ...
            <p style="margin-top: 20px;">
              <strong>Correo validado:</strong> ' . htmlspecialchars($alumno['cor1_alu']) . '<br>
              <strong>Fecha de validación:</strong> ' . date('d/m/Y H:i:s') . '
            </p>
      ...
      $mail->AltBody = 
        "Validación de Correo Electrónico, LÍDER {$alumno['nom_alu']} ✉️\n\n" .
        "Este es un correo de prueba para verificar que tu dirección de correo electrónico está configurada correctamente en nuestro sistema.\n\n" .
        "✅ Tu correo ha sido validado exitosamente\n" .
        "Si recibes este mensaje, tu correo está funcionando correctamente.\n\n" .
        ...
        "Correo validado: {$alumno['cor1_alu']}\n" .
        "Fecha de validación: " . date('d/m/Y H:i:s') . "\n\n" .
      ...
      // 🆕 RETORNAR ARRAY CON SUCCESS
      return [
        'success' => true,
        'correo' => $alumno['cor1_alu'],
        'mensaje' => 'Correo enviado correctamente'
      ];
      
    } catch (Exception $e) {
      // Registrar el error
      error_log("Error al enviar correo de validación al LÍDER {$alumno['nom_alu']} (ID: $id_alu_ram): " . $mail->ErrorInfo);
      
      // 🆕 RETORNAR ARRAY CON ERROR
      return [
        'success' => false,
        'correo' => $alumno['cor1_alu'],
        ...
        <div class="container">
          ...
          <div class="content">
            ...
            <div class="payment-confirmation">
              <p style="margin: 0; font-weight: bold; font-size: 14px; color: '.$color_verde.';">
                <span class="check-icon">✅</span>Tu matrícula ha sido generada con éxito
              </p>
...
<?php
  ...
  function calendario_pagos($id_alu_ram, $db) {
      ...
      // VALIDACIÓN TEMPRANA: Si el monto es 0, no generar registros de colegiatura
      if ($mon_alu_ram == 0) {
          // No se generan colegiaturas para alumnos con monto 0
          return;
      }
      ...
      if ($conteo_colegiaturas == 0) {
          // PRIMERA COLEGIATURA: Aplicar nueva lógica según el día de ini_gen
          $dia_ini_gen = (int)date('d', strtotime($ini_gen));
          
          if ($dia_ini_gen > 15) {
              // CASO 1: Si ini_gen es después del día 15
              // Inicio: día 27 del mismo mes de ini_gen
              // Fin: día 5 del mes siguiente
              $ini_pag_nueva = cambiarDiaFecha($ini_gen, 24);
              $fin_pag = cambiarDiaFecha(sumarUnMes($ini_gen), 5);
          } else {
              // CASO 2: Si ini_gen es día 15 o antes
              // Inicio: día 1 del mismo mes de ini_gen
              // Fin: día 5 del mismo mes de ini_gen
              $ini_pag_nueva = cambiarDiaFecha($ini_gen, 1);
              $fin_pag = cambiarDiaFecha($ini_gen, 5);
          }
      } else {
      ...
      if (strtotime($ini_pag_nueva) <= strtotime($fin_gen)) {
          ...
          // OBTENER MES Y AÑO PARA EL CONCEPTO
          $mes_numero = (int)date('m', strtotime($ini_pag_nueva));
          $año_completo = date('Y', strtotime($ini_pag_nueva));
          $año_corto = substr($año_completo, -2); // Últimos 2 dígitos del año
          
          // Obtener nombre del mes y tomar primeras 3 letras
          $nombre_mes = getMonth($mes_numero);
          $mes_abreviado = strtoupper(substr($nombre_mes, 0, 3));
          
          // Generar concepto en el formato: COLEGIATURA MAR25
          $con_pag = 'COLEGIATURA-' .$mes_abreviado.'-'.$año_corto;
...
// FUNCIONES NODOS HIJOS ESTRUCTURAS + METAS
// ============================================================================
// FUNCIONES NODOS HIJOS ESTRUCTURAS + METAS
// ============================================================================
// VERSIÓN ACTUALIZADA - COLORES CONSISTENTES
// ============================================================================

/**
 * Obtiene la clase CSS según el estado de la meta
 */
function obtener_clase_estado_meta($logrado, $meta, $fechaLimite) {
  $hoy = date('Y-m-d');
  if ($logrado >= $meta) {
...
/**
* Obtiene las metas totalizadas de un plantel
*/
function obtener_metas_totalizadas_plantel($db, $id_pla, $inicio, $fin) {
  $resultado = array(
      'Cita' => array('meta' => 0),
  ...
  $sql = "
      SELECT m.rub_met, SUM(m.can_met) as total_meta
      FROM meta m
      INNER JOIN ejecutivo e ON e.id_eje = m.id_eje5
      WHERE e.id_pla = '$id_pla'
      AND e.eli_eje = 'Activo'
      AND e.tip_eje = 'Ejecutivo'
      AND DATE(m.reg_met) BETWEEN '$inicio' AND '$fin'
      GROUP BY m.rub_met
  ";
  ...
  if ($query) {
      while ($fila = mysqli_fetch_assoc($query)) {
          ...
          if (isset($resultado[$rubro])) {
              $resultado[$rubro]['meta'] = intval($fila['total_meta']);
          }
      }
  }
...
/**
* ============================================================================
* OBTENER INDICADORES DE EJECUTIVO
* ============================================================================
* COLORES ORIGINALES:
* - CITAS:           Naranja #FF9800 (texto blanco)
* - CITAS EFECTIVAS: Rosa #FFC0CB (texto rojo #FF0000)
* - REGISTROS:       Cyan #00FFFF (texto negro)
* - CONTACTOS:       Verde claro #90EE90 (texto negro)
* ============================================================================
*/
function obtener_indicadores_ejecutivo($db, $id_eje, $inicio, $fin) {
  $html = '';
  ...
  $resultCitas = mysqli_query($db, $sqlCitas);
  $citas = 0;
  if ($resultCitas) {
      $filaCitas = mysqli_fetch_assoc($resultCitas);
      $citas = isset($filaCitas['total']) ? intval($filaCitas['total']) : 0;
  }
  
  // ========================================================================
  // OBTENER CITAS EFECTIVAS
  // ========================================================================
  $sqlCitasEfectivas = "SELECT obtener_citas_efectivas_ejecutivo($id_eje, '$inicio', '$fin') AS total";
  $resultCitasEfectivas = mysqli_query($db, $sqlCitasEfectivas);
  $citasEfectivas = 0;
  if ($resultCitasEfectivas) {
      $filaCitasEfectivas = mysqli_fetch_assoc($resultCitasEfectivas);
      $citasEfectivas = isset($filaCitasEfectivas['total']) ? intval($filaCitasEfectivas['total']) : 0;
  }
  ...
  $resultRegistros = mysqli_query($db, $sqlRegistros);
  $registros = 0;
  if ($resultRegistros) {
      $filaRegistros = mysqli_fetch_assoc($resultRegistros);
      $registros = isset($filaRegistros['total']) ? intval($filaRegistros['total']) : 0;
  }
  ...
  $resultContactos = mysqli_query($db, $sqlContactos);
  $contactos = 0;
  if ($resultContactos) {
      $filaContactos = mysqli_fetch_assoc($resultContactos);
      $contactos = isset($filaContactos['total']) ? intval($filaContactos['total']) : 0;
  }
  
  // ========================================================================
  // OBTENER METAS DEL EJECUTIVO
  // ========================================================================
  $sqlMetas = "
      SELECT id_met, rub_met, SUM(can_met) as meta_total 
      FROM meta 
      WHERE id_eje5 = '$id_eje' 
  ...
  $metaIds = array();
  if ($resultMetas) {
      while ($filaMeta = mysqli_fetch_assoc($resultMetas)) {
          $metas[$filaMeta['rub_met']] = intval($filaMeta['meta_total']);
          $metaIds[$filaMeta['rub_met']] = $filaMeta['id_met'];
      }
  }
  
  // ========================================================================
  // SI NO HAY MÉTRICAS NI METAS, NO MOSTRAR NADA
  // ========================================================================
  if (count($metas) == 0 && $citas == 0 && $citasEfectivas == 0 && $registros == 0 && $contactos == 0) { 
      return $html; 
  }
  
  // ========================================================================
  // BADGE DE CITAS - NARANJA #FF9800 (texto blanco)
  // ========================================================================
  if ($citas > 0 || isset($metas['Cita'])) {
      if (isset($metas['Cita'])) {
          $clase = obtener_clase_estado_meta($citas, $metas['Cita'], $fin);
          $emoji = ($citas >= $metas['Cita']) ? '✅' : '🎯';
          $html .= '<span class="badge badge-meta-clickable ' . $clase . '" style="background-color: #FF9800; color: white; font-size: 11px; padding: 2px 5px; cursor: pointer;" data-id-eje="' . $id_eje . '" data-id-met="' . $metaIds['Cita'] . '">' . $emoji . $citas . '/' . $metas['Cita'] . '</span>';
      } else {
  ...
  // ========================================================================
  // BADGE DE CITAS EFECTIVAS - ROSA #FFC0CB (texto rojo #FF0000)
  // ========================================================================
  if ($citasEfectivas > 0 || isset($metas['CitaEfectiva'])) {
      if (isset($metas['CitaEfectiva'])) {
          $clase = obtener_clase_estado_meta($citasEfectivas, $metas['CitaEfectiva'], $fin);
          $emoji = ($citasEfectivas >= $metas['CitaEfectiva']) ? '✅' : '🎯';
          $html .= '<span class="badge badge-meta-clickable ' . $clase . '" style="background-color: #FFC0CB; color: #FF0000; font-size: 11px; padding: 2px 5px; cursor: pointer;" data-id-eje="' . $id_eje . '" data-id-met="' . $metaIds['CitaEfectiva'] . '">' . $emoji . $citasEfectivas . '/' . $metas['CitaEfectiva'] . '</span>';
      } else {
  ...
  // ========================================================================
  // BADGE DE REGISTROS - CYAN #00FFFF (texto negro)
  // ========================================================================
  if ($registros > 0 || isset($metas['Registro'])) {
      if (isset($metas['Registro'])) {
          $clase = obtener_clase_estado_meta($registros, $metas['Registro'], $fin);
          $emoji = ($registros >= $metas['Registro']) ? '✅' : '🎯';
          $html .= '<span class="badge badge-meta-clickable ' . $clase . '" style="background-color: #00FFFF; color: black; font-size: 11px; padding: 2px 5px; cursor: pointer;" data-id-eje="' . $id_eje . '" data-id-met="' . $metaIds['Registro'] . '">' . $emoji . $registros . '/' . $metas['Registro'] . '</span>';
      } else {
  ...
  // ========================================================================
  // BADGE DE CONTACTOS - VERDE CLARO #90EE90 (texto negro)
  // ========================================================================
  if ($contactos > 0 || isset($metas['Contacto'])) {
      if (isset($metas['Contacto'])) {
          $clase = obtener_clase_estado_meta($contactos, $metas['Contacto'], $fin);
          $emoji = ($contactos >= $metas['Contacto']) ? '✅' : '🎯';
          $html .= '<span class="badge badge-meta-clickable ' . $clase . '" style="background-color: #90EE90; color: black; font-size: 11px; padding: 2px 5px; cursor: pointer;" data-id-eje="' . $id_eje . '" data-id-met="' . $metaIds['Contacto'] . '">' . $emoji . $contactos . '/' . $metas['Contacto'] . '</span>';
      } else {
          $html .= '<span class="badge" style="background-color: #90EE90; color: black; font-size: 11px; padding: 2px 5px;">' . $contactos . '</span>';
      }
  }
...
/**
* ============================================================================
* GENERAR NODOS HIJOS (RECURSIVO)
* ============================================================================
*/
function generarNodosHijos($idPadre, $db, $inicio, $fin, $puedeVerPlanteles) {
  ...
  if (mysqli_num_rows($resultadoHijos) > 0) {
      ...
      while ($filaHijos = mysqli_fetch_assoc($resultadoHijos)) {
          ...
          echo obtener_indicadores_ejecutivo($db, $id_eje, $inicio, $fin);
          
          generarNodosHijos($filaHijos['id_eje'], $db, $inicio, $fin, $puedeVerPlanteles);
...
// ============================================================================
// FIN FUNCIONES DE METAS Y NODOS HIJOS
// ============================================================================

// F FUNCIONES NODOS HIJOS ESTRUCTURAS + METAS
