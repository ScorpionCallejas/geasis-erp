...
<script type="text/javascript">
    ...
    var colHeaders = ['ID', 'NOMBRE ASIGNATURA', 'NIVEL ASIGNATURA'];
...
<?php
    ...
    if ( $total != 0 ) {
        ...
        while($fila = mysqli_fetch_assoc($resultado)){
            $id_mat = json_encode($fila['id_mat']);
            $nom_mat = json_encode($fila['nom_mat']);
            $cic_mat = json_encode($fila['cic_mat']);
         
            
            echo "data.push([$id_mat, $nom_mat, $cic_mat]);\n";
        }
    ...
    function guardarCelda(hot, row, column, value) {
        ...
        if ( id == "" || id == null || id == undefined ) {
            ...
            if(valor){
                ...
                $.ajax({
                    url: 'server/controlador_materias.php',
                    type: 'POST',
                    ...
                    //success
                        //console.log(data);
                        hot.setDataAtCell(row, 0, data.id_mat);
                        hot.setDataAtCell(row, 1, data.nom_mat);
                        hot.setDataAtCell(row, 2, data.cic_mat);
