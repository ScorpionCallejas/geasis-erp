...
<div class="table-responsive">
    ...
    <table class="table table-bordered sin-padding-margin-table" id="tabla_reporte_general_citas" style="padding: 0; border: 3px solid black; font-weight: bold;">
        <thead style="background: blue;">
            ...
            <tr>
                <td>EJECUTIVO</td>
                <td>CITAS <?php echo fechaDia( $inicio ).' '.obtenerNumeroDiaFecha( $inicio ); ?></td>
                <td>CITAS <?php echo fechaDia(sumarDias( $inicio, 1 )).' '.obtenerNumeroDiaFecha( sumarDias( $inicio, 1 ) ) ?></td>
                ...
                ?>
                        ...
                        ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php
                                            ...
                                            $sqlCitasHoy = "
                                                SELECT *
                                                FROM cita
                                                WHERE id_eje3 = $id_eje_plantel AND cla_cit = 'Cita'
                                                AND DATE(fec_cit) = DATE( '$inicio' ) AND DATE( cit_cit ) = DATE('$inicio')
                                            ";
                                    ...
                                    <td>
                                        <?php
                                            $diaManana = sumarDias( $inicio, 1 );
                                            $sqlCitasManana = "
                                                SELECT *
                                                FROM cita
                                                WHERE id_eje3 = $id_eje_plantel AND cla_cit = 'Cita'
                                                AND DATE(fec_cit) = DATE( '$inicio' ) AND DATE( cit_cit ) = DATE('$diaManana')
                                            ";

                                            // echo $sqlCitasHoy;

                                            $datosTotalCitasManana = obtener_datos_consulta( $db, $sqlCitasManana );
                                            $totalCitasManana = $datosTotalCitasManana['total'];
                                            $totalCitas['manana'] = $totalCitas['manana'] + $totalCitasManana;


                                            // EXTRACCION DE ids DE CITAS
                                            $resultadoCitasManana = mysqli_query($db, $sqlCitasManana);

                                            $contador = 0;
                                            while( $filaCitasManana = mysqli_fetch_assoc( $resultadoCitasManana ) ){
                                                $datos_citas_manana[$contador] = $filaCitasManana['id_cit']; 
                                                $contador++;
                                            }

                                            // F EXTRACCIOND DE ids DE CITAS
                                            echo $totalCitasManana;
                                    ...
                                    <!-- CONTEOS EJECUTIVO 15< -->
                                        ...
                                        <?php
                                            $citas_efectivas = 0;
                                            $max_hoy = min(17, sizeof($datos_citas_hoy));
                                            $max_manana = min(17 - $max_hoy, sizeof($datos_citas_manana));

                                            for($i = 0; $i < $max_hoy; $i++){
                                                $id_cit = $datos_citas_hoy[$i];
                                                $sql = "SELECT * FROM cita WHERE id_cit = $id_cit";
                                                $resultado = mysqli_query($db, $sql);

                                                $fila = mysqli_fetch_assoc($resultado);

                                                if($fila['est_cit'] == 'Asesoria Realizada'){
                                                    $citas_efectivas++;
                                                    echo '<td style="background: #76FF03;">CE</td>';
                                                } else if($fila['est_cit'] == 'Cita No atendida') {
                        ...
                        <tr style="border: 3px solid blue; background-color: #0070CB;">
                            ...
                            <td><?php echo $totalCitas['manana']; ?></td>      
