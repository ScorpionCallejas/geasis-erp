...
<?php 
  ...
  // VALIDACIÓN PRINCIPAL: ¿El ejecutivo tiene planteles asignados?
  $sqlValidarPlanteles = "SELECT * FROM planteles_ejecutivo WHERE id_eje = '$id'";
  $resultadoValidacion = mysqli_query($db, $sqlValidarPlanteles);
  $tieneMultiplesPlanteles = mysqli_num_rows($resultadoValidacion);
  
  if( $tieneMultiplesPlanteles > 0 ){
  	// OBTENER TODOS LOS EJECUTIVOS DE LOS PLANTELES ASIGNADOS
  	$contador = 0;
  	$sqlEjecutivos = '';
  	while($filaPlanteles = mysqli_fetch_assoc($resultadoValidacion)) {
  		if ($contador > 0) {
  			$sqlEjecutivos .= ' OR ';
  		}
  	...
  	$sql = "
  		SELECT DISTINCT 
  		    ...
  		    cita.nom_cit,
  		    cita.cla_cit,
  		    obtener_registro_por_cita(cita.id_cit) AS es_registro,
  		    IF(cita.id_cit IS NOT NULL, 'true', 'false') AS res
  		FROM contacto
  		...
  		WHERE contacto.cla_con = 'Referido' AND DATE(fec_con) BETWEEN '$inicio' AND '$fin' AND ( $sqlEjecutivos )
  	";
  	
  } else if( $rangoUsuario == 'GC' ){
  	$sql = "
  		SELECT DISTINCT 
  		    contacto.id_con, 
  		    contacto.est_con, 
...
<script type="text/javascript">
	...
	if (typeof statusColorConfig === 'undefined') {
		var statusColorConfig = {
			...
			'Cita agendada': { color: '#00FFBB', textColor: '#000000' },
...
?>
	...
	function validarTelefonoMX(telefono) {
		...
		return {
			...
			numeroLimpio: limpio.replace(/^\+52/, ''),
			formato: limpio
		};
	}
	...
	hot = new Handsontable(container, {
		...
		columns: [
			...
			{
				type: 'dropdown',
				source: ['PREPA-6-MESES', 'PREPA-1-MES', 'DIPLOMADO', 'LICENCIATURA', 'BACH-NEGOCIOS', 'SEMINARIO-LICENCIATURA']
			},
			{
				type: 'dropdown',
				source: ['Pendiente', 'No respondio', 'No se cerró', 'Cita no atendida', 'Cita agendada', 'Cita atendida', 'Contactado'],
