...
<script type="text/javascript">
	...
	// PASO 1
	// CAMBIO DE HEADERS
	var colHeaders = [ '<?php echo strtoupper(fechaFormateadaCompacta2($inicio)); ?>', 'HORARIO', 'FECHA', 'AGENDO', 'EJECUTIVO', 'CONSULTOR', 'TIPO DE CITA', 'NOMBRE', 'EDAD', 'NUMERO', 'MODALIDAD', 'MERCADO', 'ESTATUS', 'CALIDAD DE CITA', 'OBSERVACIONES', 'FOLIO', 'ID_ALU', 'BOOL1', 'BOOL2', 'CAM_CIT', 'LIG_STR_CIT' ];
		...
		if(isset($_POST['palabra']) && !empty($_POST['palabra'])) {
			...
			if($totalValidacion == 0) {
				$sql = "
					...
					AND id_eje3 IN (
						...
						WHERE (usu_eje IS NOT NULL OR ran_eje = 'DM') 
						AND eli_eje = 'Activo' AND tip_eje = 'Ejecutivo'
			...
			if($escala == 'plantel') {
				...
				$sqlEjecutivosPlantel = "
					...
					AND ((usu_eje IS NOT NULL OR ran_eje = 'DM') OR (usu_eje IS NULL AND ran_eje = 'GR'))
				";
		...
		if (!empty($citas)) {
			foreach ($citas as $fila) {
				...
				$data[] = [
					...
					isset($fila['lig_str_cit']) ? $fila['lig_str_cit'] : null, // 20 - NUEVO
    				"--" // 21
				];
...
?>
	...
	function firstColumnRenderer2(instance, td, row, col, prop, value, cellProperties) {
		...
		const id_cit = instance.getDataAtCell(row, 15); // id_cit está en la columna 15
		const estatus = instance.getDataAtCell(row, 12); // Estatus de la cita
		const lig_str_cit = instance.getDataAtCell(row, 20); // Liga de pago en columna 20
	...
	function abrirModalLigaPago(id_cit) {
		...
		for (var i = 0; i < totalRows; i++) {
			if (hot.getDataAtCell(i, 15) == id_cit) {
	...
	hot = new Handsontable(container, {
		...
		cells: function deshabilitarFila(row, col) {
			...
			if (col === 12) {
				cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
					...
					const status = window.statusConfig.getStatusByLabel(value);
			...
			items: {
				...
				"liga_pago": {
					name: function() {
						...
						if (selected) {
							var row = selected[0][0];
							var lig_str_cit = this.getDataAtCell(row, 20);
		...
		columns: [
			...
			},
			{
				...
				// MODALIDAD
				type: 'dropdown',
				source: ['SEMINARIO-LICENCIATURA', 'PREPA-6-MESES', 'PREPA-EMPRENDE', 'PROYECTO', 'DIPLOMADO', 'LICENCIATURA', 'BACH-NEGOCIOS']  //
			},
			{
				//11
				// MERCADO
			...
			},
			{
				...
				// ESTATUS
				type: 'dropdown',
				source: window.statusConfig.getActiveStatuses().map(status => status.label)
			},
	...
	$('#badges-container').html(window.statusConfig.generateStatusBadgesHTML());
	...
	function obtenerConteosEstatus(hot) {
		...
		const posiblesEstados = window.statusConfig.getActiveStatuses().reduce((acc, status) => {
			acc[status.label] = 0;
			return acc;
		}, {});
		...
		window.statusConfig.getActiveStatuses().forEach(status => {
