...
<script type="text/javascript">
	...
	obtener_tarjeta_ejecutivo( '<?php echo $id; ?>', '<?php echo $inicio; ?>', '<?php echo $fin; ?>' );
...
<?php
	...
	while ($startTime <= $endTime) {
		...
		if( $rangoUsuario == 'DC' || $rangoUsuario == 'GC' || $permisos == 2 ){
			$sql = "
				SELECT *,
				...
				WHERE cla_cit = 'Cita' AND hor_cit >= '$currentHour' AND hor_cit < '$nextHour' AND 
				( 
					( tel_cit LIKE '%$palabra%' )
				)
			...
			$sql = "
				...
				LEFT JOIN ejecutivo AS ejecutivo_cerrador ON ejecutivo_cerrador.id_eje = cita.id_eje_cerrador
				LEFT JOIN ejecutivo AS ejecutivo_agendo ON ejecutivo_agendo.id_eje = cita.id_eje_agendo
				WHERE cla_cit = 'Cita' AND ejecutivo.id_pla = '$plantel' AND hor_cit >= '$currentHour' AND hor_cit < '$nextHour' AND 
				( 
					( tel_cit LIKE '%$palabra%' )
				)
				ORDER BY hor_cit ASC
...
<?php 
	...
	hot = new Handsontable(container, {
		...
		columns: [
			...
			},
			{
				...
				// ESTATUS
				type: 'dropdown',
				source: ['Agendada', 'Asesoria Realizada', 'En Seguimiento', 'Dejo de Responder', 'Cita No atendida', 'Cita reagendada', 'Perdido por Precio', 'Perdido por horarios', 'Perdido / Futuro Prospecto', 'Pago Esperado', 'Registro', 'Cita confirmada', 'No le interesa']
			},
	...
	function actualizarBarra() {
		...
		var porcentajeRegistros = total > 0 ? Math.floor((registros / total) * 100) : 0;
			...
			// if( column != 1 && column != 13 ){
				...
				if ( campo == 'est_cit' && valor == 'Registro' ) {
					...
					swal({
						...
						icon: "warning",
						text: 'Necesitas permisos de SALES MANAGER para continuar',
						content: {
							element: "input",
							attributes: {
								placeholder: "Correo de acceso:",
								type: "text",
								id: "correo-input",  // Agrega un ID al input
						...
						onOpen: () => {
							setTimeout(() => {
								document.getElementById('correo-input').focus();
							}, 500); // 0.5 segundos
						}
					})
					.then(correo => {
						if (correo) {
							var correo = correo;
							$.ajax({
								url: 'server/validacion_permisos_comerciales.php',
								type: 'POST',
								data: { correo },
										...
										swal({
											...
											content: {
												...
												attributes: {
													...
													type: "password",
													id: "password-input",  // Agrega un ID al input
													autocomplete: "off",  // Desactiva el autocompletado
													value: ""  // Asegura que el valor sea vacío
												},
											},
											button: {
												text: "Validar",
											...
											onOpen: () => {
												setTimeout(() => {
													document.getElementById('password-input').value = ""; // Asegura que el valor sea vacío
												}, 500); // 0.5 segundos
											}
										})
										.then(password => {
											if (password) {
														...
														if (respuesta == 'True') {
															...
															.then((value) => {
																...
																$.ajax({
																	url: 'server/controlador_cita2.php',
																	type: 'POST',
																	data: { campo, valor, accion, id_cit, correo, password },
																	dataType: 'json',
																	success: function( data ){
												...
												swal({
													title: "¡Datos vacíos!",
													text: 'Necesitas ingresar un password valido...',
													icon: "error",
													button: "Aceptar",
												});
												swal.stopLoading();
							...
							swal({
								title: "¡Datos vacíos!",
								text: 'Necesitas ingresar tu correo de acceso...',
								icon: "error",
								button: "Aceptar",
							});
							swal.stopLoading();
