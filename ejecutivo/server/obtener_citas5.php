...
<script type="text/javascript">
	...
	// PASO 1
	// CAMBIO DE HEADERS
	var colHeaders = [ '<?php echo strtoupper(fechaFormateadaCompacta2($inicio)); ?>', 'HORARIO', 'FECHA', 'AGENDO', 'EJECUTIVO', 'CONSULTOR', 'TIPO DE CITA', 'NOMBRE', 'EDAD', 'NUMERO', 'MODALIDAD', 'MERCADO', 'ESTATUS', 'CALIDAD DE CITA', 'OBSERVACIONES', 'FOLIO', 'ID_ALU', 'BOOL1', 'BOOL2', 'CAM_CIT' ];
...
<?php
	...
	while ($startTime <= $endTime) {
			...
			if($escala == 'plantel') {
				...
				if ( $id_eje == 'Todos' ) {
					...
					$sql = "
						...
						LEFT JOIN ejecutivo AS ejecutivo_cerrador ON ejecutivo_cerrador.id_eje = cita.id_eje_cerrador
						LEFT JOIN ejecutivo AS ejecutivo_agendo ON ejecutivo_agendo.id_eje = cita.id_eje_agendo
						WHERE cla_cit = 'Cita' AND id_eje3 = '$id_eje' 
						AND DATE(cit_cit) BETWEEN DATE('$inicio') AND DATE('$fin') 
						AND hor_cit >= '$currentHour' AND hor_cit < '$nextHour'
					";
				}
		...
		if (!empty($citas)) {
			foreach ($citas as $fila) {
				...
				// variable temporal para lidiar con el cambio de citas
				// definido en 1000 para cambios dinámicos en col 13 (efect de cita)
	...
	foreach ($data as $fila) {
		if (isset($fila[15]) && is_numeric($fila[15]) && $fila[15] > 0) {
			$ids_citas[] = intval($fila[15]);
		}
	}
...
?>
	...
	function firstColumnRenderer(instance, td, row, col, prop, value, cellProperties) {
		...
		if (col === 15 && !isNaN(value) && value !== null && value !== '') {
			const id_cit = instance.getDataAtCell(row, 15); // Asumiendo que el id_cit está en la columna 15
	...
	hot = new Handsontable(container, {
		...
		cells: function deshabilitarFila(row, col) {
			...
			// Validador para filas sin id_alu_ram (id_alu_ram_aux en la columna 17)
			...
			// Validar si `cam_cit` es 0 y hacer la columna 13 (efe_cit) readOnly
			if (col === 13) {
				...
				// Aplicar el renderer para columna 13
				cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
					Handsontable.renderers.TextRenderer.apply(this, arguments);
			...
			} else if (col === 15 <?php echo ( ( $rangoUsuario != 'GC' && $rangoUsuario != 'TL' ) && $permisos != 1 && $permisos != 2  )? ' || col === 5': ''; ?> <?php echo ( $rangoUsuario != 'GC' || $rangoUsuario != 'DC'  )? ' || col === 12': ''; ?> <?php echo ( $rangoUsuario != 'DM' && $rangoUsuario != 'GC' && $rangoUsuario != 'DC' ) ? ' || col === 3': ''; ?> ) { 
				cellProperties.renderer = firstColumnRenderer;
			}

			// Configuración de estilos para la columna 12 (estatus de la cita)
			if (col === 12) {
				cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
					Handsontable.renderers.TextRenderer.apply(this, arguments);
			...
			// Verificar si existe color personalizado para esta celda
			var id_cit = this.instance.getDataAtCell(row, 15);
			...
			if (tieneColorPersonalizado) {
				...
				var tieneRegistro = (est_cit === 'REGISTRO' && this.instance.getDataAtCell(row, 16) == 'true');
				var tieneErrorRegistro = (this.instance.getDataAtRowProp(row, 17) === 'false' && est_cit === 'REGISTRO' && this.instance.getDataAtRowProp(row, 16) == 'true');
				
				// No aplicar color personalizado si tiene estilos del sistema
				if (!tieneRegistro && !tieneErrorRegistro && col !== 12 && col !== 13) {
					var rendererOriginal = cellProperties.renderer;
					cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
						if (rendererOriginal) {
							rendererOriginal.apply(this, arguments);
						} else {
							Handsontable.renderers.TextRenderer.apply(this, arguments);
						}
						// Aplicar el color personalizado
						td.style.backgroundColor = coloresPastel[colorPersonalizado];
					};
				}
			} else {
				// IMPORTANTE: Si no hay color personalizado, asegurar que se limpie cualquier color previo
				if (id_cit && campo && col !== 12 && col !== 13) {
					var est_cit = this.instance.getDataAtCell(row, 12);
					var tieneRegistro = (est_cit === 'REGISTRO' && this.instance.getDataAtCell(row, 16) == 'true');
					var tieneErrorRegistro = (this.instance.getDataAtRowProp(row, 17) === 'false' && est_cit === 'REGISTRO' && this.instance.getDataAtRowProp(row, 16) == 'true');
					...
					if (!tieneRegistro && !tieneErrorRegistro) {
						cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
							Handsontable.renderers.TextRenderer.apply(this, arguments);
							// Limpiar cualquier color de fondo personalizado
							td.style.backgroundColor = '';
						};
					}
				}
						...
						if (selected) {
							...
							if (row === editRow) {
								...
								if (this.getDataAtCell(row, 12) === 'REGISTRO' && 
									this.getDataAtCell(row, 16) === 'true') {
									editRow = row;
				...
				"pintar_celda": {
					...
					disabled: function() {
						...
						if (selected) {
							...
							var id_cit = this.getDataAtCell(row, 15);
							
							// Deshabilitar si no hay id_cit válido o si es columna 12 (est_cit) o 13 (efe_cit)
		...
		columns: [
			...
			},
			{
				...
				// MODALIDAD
				type: 'dropdown',
				source: ['PREPA-6-MESES', 'PREPA-EMPRENDE', 'DIPLOMADO', 'LICENCIATURA', 'BACH-NEGOCIOS']  //
			},
			{
				//11
				// MERCADO
	...
	function guardarCelda(hot, row, column, value) {
		...
		var cam_cit = hot.getDataAtCell( row, 19 );
				...
				if(valor){
					...
					$.ajax({
						...
						//success
							...
							hot.setDataAtCell(row, 19, data.cam_cit);
				...
				// NO REGISTRO
					...
					$.ajax({
						...
						success: function(data) {
							...
							if (column == 13) {
								var cam_cit = hot.getDataAtCell(row, 19); // Obtener el valor actual de cam_cit
								
								if (cam_cit > 0) {
									hot.setDataAtCell(row, 19, cam_cit - 1); // Decrementar cam_cit en la tabla
									cam_cit -= 1; // Actualizar variable local para la verificación siguiente
								}

								// Verificar si cam_cit es 0 y hacer la columna 13 readOnly
								if (cam_cit === 0) {
									hot.getCellMeta(row, 13).readOnly = true; // Hacer la columna 13 readOnly
	...
	function aplicarColorCelda(color) {
		...
		if (selected) {
			var row = selected[0][0];
			var col = selected[0][1];
			var id_cit = hot.getDataAtCell(row, 15);
			var campo = obtenerCampoValor(col);
				...
				success: function(response) {
					...
					if (response.resultado === 'exito') {
						...
						if (color === 'quitar') {
							...
							// Verificar si el color sigue en la variable
							var verificacion = coloresCeldas[id_cit] && coloresCeldas[id_cit][campo];
							console.log('¿El color todavía existe?:', verificacion);
						...
						// Verificar el estado después del render
						setTimeout(function() {
							console.log('=== VERIFICACIÓN POST-RENDER ===');
							console.log('coloresCeldas después del render:', coloresCeldas);
							
							// Verificar si la celda sigue teniendo color
							var celdaTieneColor = coloresCeldas[id_cit] && coloresCeldas[id_cit][campo];
							console.log('¿La celda [' + id_cit + '][' + campo + '] tiene color?:', celdaTieneColor);
							
							// Forzar otro render si es necesario
							if (color === 'quitar' && celdaTieneColor) {
								console.log('ERROR: El color no se quitó, forzando render adicional');
								hot.updateSettings({});
							}
						}, 50);
