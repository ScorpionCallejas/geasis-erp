...
?>
	...
	hot = new Handsontable(container, {
		...
		cells: function deshabilitarFila(row, col) {
			...
			if (this.instance.getDataAtRowProp(row, 18) === 'false' && this.instance.getDataAtRowProp(row, 12) === 'REGISTRO' && this.instance.getDataAtRowProp(row, 17) == 'true') {
				...
				if (this.instance.getDataAtRowProp(row, 12) === 'REGISTRO' && this.instance.getDataAtRowProp(row, 17) == 'true') {
					if (row === editRow) {
			...
			} else if (col === 16 <?php echo ( ( $rangoUsuario != 'GC' && $rangoUsuario != 'TL' ) && $permisos != 1 && $permisos != 2  )? ' || col === 5': ''; ?> <?php echo ( $rangoUsuario != 'GC' || $rangoUsuario != 'DC'  )? ' || col === 12': ''; ?> <?php echo ( $rangoUsuario != 'DM' && $rangoUsuario != 'GC' && $rangoUsuario != 'DC' ) ? ' || col === 3': ''; ?> ) { 
			...
			// Configuración para la columna URL (14)
			if (col === 14) {
				cellProperties.renderer = function urlRenderer(instance, td, row, col, prop, value, cellProperties) {
					if (esURLValida(value)) {
						td.innerHTML = `<a href="${value.toLowerCase()}" target="_blank" onclick="event.stopPropagation();" style="color: blue; text-decoration: none; line-height: 1; text-transform: lowercase;">${value.toLowerCase()}</a>`;
			...
			items: {
						...
						if (selected) {
							...
							if (row === editRow) {
								...
								if (this.getDataAtCell(row, 12) === 'REGISTRO' && 
									this.getDataAtCell(row, 17) === 'true') { // CORREGIDO: era 16, ahora es 17
									editRow = row;
				...
				"liga_pago": {
					name: function() {
						...
						if (selected) {
							var row = selected[0][0];
							var lig_str_cit = this.getDataAtCell(row, 21); // CORREGIDO: era 20, ahora es 21
					...
					callback: function() {
						...
						if (selected) {
							var row = selected[0][0];
							var id_cit = this.getDataAtCell(row, 16); // CORREGIDO: era 15, ahora es 16
								...
								try {
									...
									if (selection && selection.length > 0) {
										...
										if (selectedRow !== undefined && selectedRow >= 0) {
											var value = this.getDataAtCell(selectedRow, 16); // CORREGIDO: era 15, ahora es 16
