<?php 
    ...
    if ( isset( $_POST['id_gen'] ) ) {
        ...
        $datos_alu_ram = obtener_datos_consulta( $db, $sql_alu_ram )['datos'];
        $id_alu = $datos_alu_ram['id_alu1'];
        ...
        $bec_alu_ram = 0;
        $bec2_alu_ram = 0;
        $id_pla1 = $filaRama['id_pla1'];


        $id_alu_ram = $datos_alu_ram['id_alu_ram'];

        echo 'id_alu_ram: '.$id_alu_ram;

        $mon_alu_ram = $datos_alu_ram['mon_alu_ram'];
        if (empty($mon_alu_ram) || $mon_alu_ram === null || $mon_alu_ram === '') {
            $mon_alu_ram = 0;
        }
        
        $act_alu_ram = date('Y-m-d');

        $sql = "INSERT INTO alu_ram (est2_alu_ram, mon_alu_ram, id_alu1, id_gen1, id_ram3, act_alu_ram, bec_alu_ram, bec2_alu_ram, car_alu_ram ) VALUES ('Inactivo', '$mon_alu_ram', '$id_alu', '$id_gen', '$id_ram', '$act_alu_ram', '$bec_alu_ram', '$bec2_alu_ram', '$car_alu_ram' )";

        $resultado = mysqli_query($db, $sql);

        if ($resultado) {

            // UPDATE DE plantel A alumno
            $sqlUpdateAlumno = "
                UPDATE alumno
                SET
                id_pla8 = $id_pla1
            ...
            // TRASLADAR PAGOS A NUEVO PROGRAMA
            if ( isset( $_POST['trasladarPagos'] ) && ( $_POST['trasladarPagos'] == 'true' ) ) {
                echo 'entré a traslado de pagos';
                $sqlPagos = "
                    SELECT *
                    FROM pago
            ...
            // ELIMINACION DE id_alu_ram PREVIO
            if( isset( $_POST['eliminacion_alu_ram'] ) ){
                $sqlEliminacion = "DELETE FROM alu_ram WHERE id_alu_ram = $id_alu_ram";
                $resultadoEliminacion = mysqli_query( $db, $sqlEliminacion );
                if(!$resultadoEliminacion){
                    echo $sqlEliminacion;
                }
            }
            // F ELIMINACION DE id_alu_ram PREVIO
